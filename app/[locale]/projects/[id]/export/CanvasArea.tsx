...
export function CanvasArea(s: ExportPageState) {
  ...
  const _startMomentum = React.useCallback((initVx: number, initVy: number) => {
    ...
    const MARGIN = 120
    const DECAY = 0.95    // iOS 原生手感，比0.93更顺滑
    const MIN_V = 0.02    // px/ms 停止阈值（~1.2px/frame @60fps）

    // initVx/initVy 是 px/ms，转成每帧增量（基准60fps = 16.67ms/frame）
    let vx = initVx * 16.67
    let vy = initVy * 16.67
    ...
    const loop = () => {
      ...
      if (Math.abs(vx) < MIN_V * 16.67 && Math.abs(vy) < MIN_V * 16.67) {
  ...
  // React synthetic touch events are PASSIVE on modern browsers — e.preventDefault()
  // inside onTouchMove/onTouchStart does nothing, so the browser runs its own
  // scroll/zoom simultaneously and fights our JS pan. We must use native listeners
  // with { passive: false } to actually suppress browser default behaviour.
  ...
  React.useEffect(() => {
    ...
    const onNativeTouchStart = (e: TouchEvent) => {
      ...
      if (e.touches.length === 1) {
        ...
        const panLayerEl2 = panLayerRef.current
        const liveTransform2 = panLayerEl2?.style.transform ?? ''
        const mat2 = liveTransform2 && liveTransform2 !== 'none' ? new DOMMatrix(liveTransform2) : null
        const livePx2 = mat2 ? mat2.m41 : 0
        const livePy2 = mat2 ? mat2.m42 : 0
        touchPinchRef.current = { dist, midX, midY, startZoom: canvasZoomRef.current, startPanX: livePx2, startPanY: livePy2, wRect: wrap?.getBoundingClientRect() ?? null }
        e.preventDefault()
      }
    }
    ...
    const onNativeTouchMove = (e: TouchEvent) => {
      ...
      if (e.touches.length === 1 && touchPanRef.current) {
        ...
        // 手机端在 block 上：超过 8px 才接管（避免干扰点击）
        const isMobile = window.innerWidth <= 768
        if (isMobile && (target as HTMLElement).closest('.rnd-block') && Math.hypot(rawDx, rawDy) < 8) return
  ...
  setCanvasPan({ x: _mousePanPos.current.x, y: _mousePanPos.current.y })
      ...
      onTouchStart={e => {
        ...
        if (target.closest('[data-toolbar]')) return
        // 1-finger on a block → arm long-press; on mobile also init pan state
        if (target.closest('.rnd-block')) {
          ...
          // 手机端：继续往下初始化 pan state（native handler 已经处理了真正的 pan，
          // 这里只是同步 React refs，native 优先）
        }
        // 空白区域停止惯性
        if (!target.closest('.rnd-block')) _stopMomentum()
        if (e.touches.length === 1) {
          const panLayerEl = panLayerRef.current
          const liveTransform = panLayerEl?.style.transform ?? ''
          const mat = liveTransform && liveTransform !== 'none' ? new DOMMatrix(liveTransform) : null
      ...
      onTouchMove={e => {
        ...
        // native handler 已全权处理 transform，React 合成层不做任何额外操作
      }}
      onTouchEnd={e => {
        clearLongPress()
        if (touchPanRef.current) {
          ...
          if (buf.length > 0 && window.innerWidth <= 768) {
            ...
            buf.forEach((v, i) => {
              const idx = i + (5 - buf.length)
              const w = weights[Math.max(0, idx)] ?? 1
              wvx += v.vx * w
              wvy += v.vy * w
              wsum += w
            })
            const avgVx = wvx / wsum  // px/ms
            const avgVy = wvy / wsum
            // 阈值 0.08 px/ms ≈ 5px/frame@60fps，低于此不启动惯性（避免轻点触发）
